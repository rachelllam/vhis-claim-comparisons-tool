...
const WARD_LABELS_V2: Record<Ward, string> = { standard: 'Standard ward', 'semi-private': 'Semi-private', private: 'Private' };
const capLabelShort = (plan: VhisPlan) => (plan.perSurgery >= 999999 ? 'No cap' : fmtHKShort(plan.perSurgery));
...
function ResultCardV2({
  ...
  const feeItems = [
    { label: "Surgeon's fee", amount: Math.round(totalCost * 0.5) },
    { label: "Anaesthetist's fee", amount: Math.round(totalCost * 0.15) },
    { label: 'Operating theatre charges', amount: totalCost - Math.round(totalCost * 0.5) - Math.round(totalCost * 0.15) },
    { label: 'SMM coverage', amount: Math.round(totalCost * 0.2) },
      ...
      {showDetails && (
        <div style={ccChartStyles.receipt}>
          ...
          {ded > 0 && (
            <div style={{ ...ccChartStyles.receiptRow, ...ccChartStyles.receiptRowBorder }}>
              <span style={ccChartStyles.receiptLabel}>
                Deductible<span style={ccChartStyles.receiptHint}>You pay this first</span>
              </span>
          ...
          {overCap > 0 && (
            <div style={{ ...ccChartStyles.receiptRow, ...ccChartStyles.receiptRowBorder }}>
              <span style={ccChartStyles.receiptLabel}>
                Above plan limit<span style={ccChartStyles.receiptHint}>{capLabel}</span>
              </span>
          ...
          <div style={ccChartStyles.paysBox(zero)}>
            <span style={ccChartStyles.paysLabel}>
              Net payout<span style={ccChartStyles.paysSub}>VHIS pays the hospital / you</span>
            </span>
      ...
      <div style={ccV2.caseHead}>
        <div style={{ minWidth: 0 }}>
          ...
          <div style={ccV2.caseEn}>
            {(detail && detail.official) || caseItem.en} · {caseItem.days === 0 ? 'Day case' : `${caseItem.days} nights`}
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
          ...
          {onOpenDetail && (
            <button
              ...
              title="Internal treatment details"
    ...
    <div style={{ marginBottom: 18, paddingBottom: 18, borderBottom: '1px solid var(--bt-stone)' }}>
      <div style={ccDetail.thHead}>
        ...
        <button
          ...
          onClick={onOpenDetail}
          title="Internal-only treatment details"
          onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'var(--bt-bowtie-pink)'; e.currentTarget.style.color = 'var(--bt-bowtie-pink)'; }}
          onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'var(--bt-stone)'; e.currentTarget.style.color = 'var(--bt-bowtie-blue)'; }}
        >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        <div style={ccV2.stripCell}>
          ...
          <span style={ccDetail.rangeNote}>
            Medium estimate · {tier?.en} range {tier?.rangeLabel}
          </span>
        </div>
        <div style={ccV2.stripCell}>
          ...
          >
            {dayCase ? 'No overnight' : 'Inpatient'}
          </span>
        </div>
      </div>
    </div>
    ...
    <div style={ccChartStyles.legend}>
      {showDeductible && (
        <span style={ccChartStyles.legendItem}>
          <span style={ccChartStyles.legendSwatch('var(--bt-yellow-submarine)')}></span>Deductible · you pay first
        </span>
      )}
      <span style={ccChartStyles.legendItem}>
        <span style={ccChartStyles.legendSwatch('var(--bt-green-day)')}></span>VHIS reimburses
      </span>
      <span style={ccChartStyles.legendItem}>
        <span style={ccChartStyles.legendSwatch('var(--bt-hotel-california)')}></span>Above plan limit · you pay
      </span>
    </div>
  );
}
      ...
      return (
        <div>
          ...
          <div style={{ marginTop: 18 }}>
            <PromptBox title="Pick cases to compare" sub="Choose surgeries in the Case tab on the left" />
          </div>
        </div>
      );
    }
  ...
  return (
    <div>
      ...
      {activePlans.length === 0 && (
        <div style={{ marginTop: 18 }}>
          <PromptBox title="Pick VHIS plans" sub="Choose plans in the Plan tab on the left" />
        </div>
      )}
          ...
          <div style={ccV2.strip}>
            <div style={ccV2.stripCell}>
              ...
              <span style={ccV2.stripSmall}>
                Deductible {focus.deductible === 0 ? 'none' : fmtHK(focus.deductible)}
                {showPrem && premOf(focus.id) != null ? ` · HK$${premOf(focus.id)!.toLocaleString('en-US')}/mo` : ''}
              </span>
            </div>
            <div style={ccV2.stripCell}>
